...
def load_sents(path, n):
    if not os.path.exists(path):
        ...
        print(f"  Download command (pick one):")
        print(f"")
        print(f"  # Option A: use old datasets API")
        print(f"  pip install datasets==2.14.0")
        print(f"  python3 -c \"")
        print(f"  from datasets import load_dataset; import os")
        print(f"  ds = load_dataset('wmt14', 'de-en')")
        print(f"  os.makedirs('/data/datasets/wmt14', exist_ok=True)")
        print(f"  for split, hf in [('train','train'),('validation','validation'),('test','test')]:")
        print(f"      with open(f'/data/datasets/wmt14/wmt14.{split}.de-en','w') as f:")
        print(f"          for ex in ds[hf]: f.write(ex['translation']['de']+'\\\\t'+ex['translation']['en']+'\\\\n')")
        print(f"  \"")
        print(f"")
        print(f"  # Option B: new datasets API (namespace/name)")
        print(f"  pip install datasets")
        print(f"  python3 -c \"")
        print(f"  from datasets import load_dataset; import os")
        print(f"  ds = load_dataset('wmt/wmt14', 'de-en', trust_remote_code=True)")
        print(f"  ... same write loop as above ...")
        print(f"  \"")

...
hash = mean(token_embeddings) — preserves embedding direction
"""
...
def hash_fn(embeddings):
    return embeddings.mean(dim=0) if len(embeddings) > 0 else torch.zeros(d)

def decode(leaf_vec):
    return (leaf_vec @ E.T).argmax().item()

def build_and_test(depth):
    n_leaves, n_nodes = 1<<depth, (1<<depth)-1
    ...
    def recursively_split(items, node_idx, depth_remaining):
        ...
        if not real: return {node_idx: items}
        scores = E[[t for (t,_,_) in real]] @ node_W[node_idx]
        median = scores.median()
        left_r = [real[i] for i in range(len(real)) if scores[i] <= median]
        right_r = [real[i] for i in range(len(real)) if scores[i] > median]
        mid = len(empt)//2
        left = left_r+empt[:mid]; right = right_r+empt[mid:]
        res = {}
    ...
    for leaf_idx, items in leaf_items.items():
        real_ids = [t for (t,_,_) in items if t>=0]
        leaf_hash[leaf_idx] = hash_fn(E[real_ids]) if real_ids else torch.zeros(d)
    
    token_to_leaf = {}
    for leaf_idx, items in leaf_items.items():
        for (tid, si, pi) in items: token_to_leaf[(si, pi)] = leaf_idx
    ...
    for si, s_ids in enumerate(sent_tokens):
        ...
        for pi, tid in enumerate(s_ids):
            ...
            if leaf_idx not in leaf_cache:
                leaf_cache[leaf_idx] = decode(leaf_hash[leaf_idx])
            hyp.append(leaf_cache[leaf_idx])
    ...
    b = bleu(refs, hyps)
    sizes = [len([t for (t,_,_) in items if t>=0]) for items in leaf_items.values()]
    return b, len(leaf_items), sum(1 for s in sizes if s==1), sum(1 for s in sizes if s>1), np.mean(sizes)

print(f"\ndepth | leaves | avg/leaf | solo | multi | BLEU-4")
print("-" * 50)
for depth in [3, 4, 5]:
    b, active, solo, multi, avg = build_and_test(depth)
    print(f"  {depth:2d}   | {active:5d} |    {avg:.1f}    | {solo:4d} | {multi:4d} | {b:6.2f}")

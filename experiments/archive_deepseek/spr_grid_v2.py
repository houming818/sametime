"""网格: d×train, 带 LaBSE 预训练, BLI P@1"""
...
def run(d, n_train):
    ...
    # Load LaBSE pretrain (if d=128) or random init (d>128)
    if d==128:
    ...
    def heap_world(tok_ids):
        ...
        for l in range(td):
            ...
            w=w+t_nodes[l](nidx)
        w=F.normalize(w,dim=-1); t=F.normalize(L0.weight[tok_ids],dim=-1); tL,tR=t[...,:d//2],t[...,d//2:]; wL,wR=w[...,:d//2],w[...,d//2:]
...
for d,n in grid:
    p0,pb,tt=run(d,n)
    results[(d,n)]=(p0,pb)
    print(f"  d={d} train={n//1000}K init={p0:.1f}% best={pb:.1f}% {tt:.0f}s")
print(f"\n=== Grid ===")
for (d,n),(p0,pb) in sorted(results.items()):
    print(f"  d={d} {n//1000:2d}K  init={p0:5.1f}%  best={pb:5.1f}%")

"""
夜间实验: 有序邻域掩码 + 树节点训练
1. 基线: 冻结 tree + 3191 独立 path (已验证 97% ms)
2. 核心: unit_root×mul tree + AttnPaths + 有序掩码 (新实验)
3. 对比: 训练的 tree + AttnPaths + 有序掩码
"""
import sys; sys.path.insert(0, '.')
from core import TreeNodes, HeapWorld, AttnPaths, AnchorIndex, cmul
import torch, torch.nn as nn, torch.nn.functional as F, random, json, time
import sentencepiece as spm
from collections import defaultdict

device = 'cuda' if torch.cuda.is_available() else 'cpu'
d = 128; td = 5; V = 16000; tau = 0.07
sp = spm.SentencePieceProcessor(); sp.load('/mnt/nas/datasets/wmt17/sp_bpe.model')
def ok(ids): return all(x != 0 for x in ids)
t0 = time.time()

# ── 数据 ──
with open('/workspace/multi_sense_anchors.json') as f: dict_data = json.load(f)
with open('/workspace/labse_anchors.py') as f: labse_pairs = eval(f.read().split('=',1)[1].strip())
MANUAL = [('i','我'),('you','你'),('he','他'),('she','她'),('it','它'),('one','一'),('two','二'),('three','三'),('four','四'),
    ('five','五'),('hand','手'),('eye','眼'),('head','头'),('heart','心'),('water','水'),('fire','火'),('sun','太阳'),
    ('moon','月'),('mountain','山'),('river','河'),('sky','天'),('earth','地'),('wind','风'),('rain','雨'),('snow','雪'),('sea','海'),
    ('tree','树'),('flower','花'),('gold','金'),('iron','铁'),('star','星'),('cloud','云'),('fish','鱼'),('bird','鸟'),
    ('red','红'),('green','绿'),('white','白'),('black','黑'),('mother','母亲'),('father','父亲'),('son','儿子'),('daughter','女儿'),
    ('food','食物'),('rice','米'),('meat','肉'),('bread','面包'),('milk','牛奶'),('egg','蛋'),('fruit','水果'),
    ('day','天'),('night','夜'),('year','年'),('month','月'),('spring','春'),('summer','夏'),('autumn','秋'),('winter','冬'),
    ('go','去'),('come','来'),('eat','吃'),('drink','喝'),('see','看'),('hear','听'),('say','说'),('read','读'),
    ('write','写'),('walk','走'),('sit','坐'),('stand','站'),('give','给'),('take','拿'),('make','做'),('know','知道'),
    ('think','想'),('want','要'),('love','爱'),('like','喜欢'),('work','工作'),('play','玩'),('buy','买'),('sell','卖'),
    ('live','生活'),('die','死'),('kill','杀'),('speak','说'),('talk','谈'),('find','找'),('learn','学'),('teach','教'),
    ('help','帮助'),('open','开'),('close','关'),('start','开始'),('stop','停'),('run','跑'),('fly','飞'),
    ('remember','记住'),('forget','忘记'),('believe','相信'),('understand','理解'),('explain','解释'),('change','改变'),
    ('grow','生长'),('build','建设'),('cut','切'),('break','打破'),('push','推'),('pull','拉'),('carry','带'),('throw','扔'),
    ('catch','抓'),('follow','跟随'),('lead','领导'),('meet','见面'),('wait','等'),('need','需要'),('keep','保持'),
    ('agree','同意'),('return','返回'),('leave','离开'),('arrive','到达'),('stay','停留'),('move','移动'),
    ('pass','通过'),('rise','上升'),('fall','下降'),('lift','举起'),('burn','烧'),('choose','选择'),('decide','决定'),
    ('prepare','准备'),('finish','完成'),('enjoy','享受'),('protect','保护'),('attack','攻击'),('defend','防御'),('destroy','破坏'),
    ('create','创造'),('produce','生产'),('develop','发展'),('improve','改善'),('manage','管理'),('organize','组织'),('save','保存'),
    ('disappear','消失'),('survive','幸存'),('own','拥有'),('exist','存在'),('happen','发生'),('appear','出现'),
    ('continue','继续'),('reduce','减少'),('increase','增加'),('big','大'),('small','小'),('good','好'),('bad','坏'),
    ('new','新'),('old','老'),('hot','热'),('cold','冷'),('long','长'),('short','短'),('high','高'),('low','低'),
    ('fast','快'),('slow','慢'),('rich','富'),('strong','强'),('weak','弱'),('true','真'),('happy','幸福'),('sad','悲伤'),
    ('heavy','重'),('light','轻'),('hard','硬'),('soft','软'),('deep','深'),('wide','宽'),
    ('clean','干净'),('safe','安全'),('easy','容易'),('important','重要'),('free','自由'),('fair','公平'),
    ('bright','明亮'),('dark','黑暗'),('dry','干'),('full','满'),('empty','空'),('kind','善良'),('brave','勇敢'),('afraid','害怕'),('angry','愤怒'),]
ALL = labse_pairs + MANUAL

en_known = {}
for e, z in ALL: en_known.setdefault(e.lower(), set()).add(z)
multis = {}
for en, known in en_known.items():
    if en not in dict_data: continue
    existing = next(iter(known))
    extra = []
    for zh, pos in dict_data[en]:
        if zh not in known and pos in ('n','v','a','adj','adv','vi','vt') and 1 <= len(zh) <= 6:
            zh_ids = sp.encode_as_ids(zh)
            if ok(zh_ids): extra.append(zh)
    if extra: multis[en] = [existing] + extra[:2]
print(f"Multi-sense: {len(multis)}")

# ── Gold anchors (top level, shared by all experiments) ──
GOLD = [('i','我'),('one','一'),('two','二'),('hand','手'),('eye','眼'),('head','头'),('heart','心'),
    ('water','水'),('fire','火'),('sun','太阳'),('moon','月'),('mountain','山'),('river','河'),('sky','天'),
    ('wind','风'),('rain','雨'),('snow','雪'),('sea','海'),('tree','树'),('flower','花'),
    ('gold','金'),('iron','铁'),('star','星'),('cloud','云'),('fish','鱼'),('bird','鸟'),
    ('red','红'),('green','绿'),('white','白'),('black','黑'),('mother','母亲'),('father','父亲'),
    ('food','食物'),('rice','米'),('meat','肉'),('bread','面包'),('milk','牛奶'),('egg','蛋'),('fruit','水果'),
    ('day','天'),('night','夜'),('year','年'),('month','月'),('go','去'),('come','来'),('eat','吃'),('drink','喝'),
    ('see','看'),('hear','听'),('write','写'),('walk','走'),('sit','坐'),('stand','站'),('give','给'),('take','拿'),
    ('make','做'),('know','知道'),('think','想'),('want','要'),('love','爱'),('like','喜欢'),
    ('work','工作'),('buy','买'),('live','生活'),('die','死'),('kill','杀'),('help','帮助'),
    ('open','开'),('close','关'),('start','开始'),('stop','停'),('run','跑'),
    ('remember','记住'),('forget','忘记'),('understand','理解'),('change','改变'),
    ('grow','生长'),('build','建设'),('cut','切'),('break','打破'),('push','推'),('pull','拉'),
    ('carry','带'),('throw','扔'),('catch','抓'),('follow','跟随'),('lead','领导'),('meet','见面'),
    ('wait','等'),('need','需要'),('keep','保持'),('agree','同意'),('return','返回'),('leave','离开'),
    ('arrive','到达'),('move','移动'),('pass','通过'),('choose','选择'),('decide','决定'),
    ('prepare','准备'),('finish','完成'),('protect','保护'),('create','创造'),('produce','生产'),
    ('develop','发展'),('manage','管理'),('save','保存'),('disappear','消失'),('survive','幸存'),
    ('own','拥有'),('exist','存在'),('happen','发生'),('appear','出现'),('continue','继续'),
    ('reduce','减少'),('increase','增加'),('big','大'),('small','小'),('good','好'),('bad','坏'),
    ('new','新'),('old','老'),('hot','热'),('cold','冷'),('long','长'),('high','高'),('low','低'),
    ('strong','强'),('weak','弱'),('true','真'),('happy','幸福'),('heavy','重'),
    ('clean','干净'),('safe','安全'),('easy','容易'),('important','重要'),('free','自由'),
    ('dark','黑暗'),('empty','空'),('kind','善良'),('brave','勇敢')]

def run_experiment(name, tree, freeze_tree, use_mask, mask_radius):
    print(f"\n{'='*60}")
    print(f"EXP: {name}")
    print(f"  tree: {tree}  freeze_tree={freeze_tree}  mask={use_mask}  radius={mask_radius}")
    print(f"{'='*60}")
    
    for p in tree.parameters(): p.requires_grad = not freeze_tree
    
    hw = HeapWorld(vocab=V, dim=d, tree=tree).to(device)
    if freeze_tree:
        ckpt = torch.load('/mnt/nas/datasets/wmt17/checkpoints/anchor_tree_nce.pt', map_location=device, weights_only=True)
        hw.embedding.load_state_dict(ckpt['L0'])
        hw.merge.load_state_dict(ckpt['t_merge'])
        for p in hw.embedding.parameters(): p.requires_grad = False
        for p in hw.merge.parameters(): p.requires_grad = False
    else:
        nn.init.normal_(hw.embedding.weight, 0, 0.02)
        for p in hw.embedding.parameters(): p.requires_grad = True
        for p in hw.merge.parameters(): p.requires_grad = True
    
    anchors = AnchorIndex()
    multi_en = set(multis.keys())
    for e, z in ALL:
        if e.lower() in multi_en: continue
        ei = sp.encode_as_ids(e); zi = sp.encode_as_ids(z)
        if ok(ei) and ok(zi): anchors.add(e, z, ei, zi)
    
    attn_paths = AttnPaths(tree).to(device)
    zh_paths = {}  # ai → hard path for ordered masking
    for en, zhs in multis.items():
        en_ids = sp.encode_as_ids(en)
        if not ok(en_ids): continue
        for pi, zh in enumerate(zhs[:3]):
            zh_ids = sp.encode_as_ids(zh)
            if ok(zh_ids):
                anchors.add(en, zh, en_ids, zh_ids)
                ai = anchors.find(en)[-1]
                attn_paths.add(en, zh, ai, pi)
                zh_paths[ai] = [tree.route(torch.tensor([zh_ids[0]], device=device))[l].item() for l in range(td)]
    
    def _hw_fn(tok_ids):
        return hw.forward(torch.tensor(tok_ids, device=device).unsqueeze(0)).squeeze(0)
    anchors.build_zh_world(_hw_fn)
    
    single_ai = [ai for ai, (en, _, _, _) in enumerate(anchors.entries) 
                 if en not in multi_en or en not in multis]
    print(f"  anchors={len(anchors)} single={len(single_ai)} paths={attn_paths.num_paths}")
    
    ge_r = [sp.encode_as_ids(e) for e,_ in GOLD if ok(sp.encode_as_ids(e))]
    gz_r = [sp.encode_as_ids(z) for _,z in GOLD if ok(sp.encode_as_ids(z))]
    gw_r = torch.zeros(len(gz_r), d, device=device)
    with torch.no_grad():
        for i, zi in enumerate(gz_r):
            gw_r[i] = F.normalize(hw.forward(torch.tensor(zi, device=device).unsqueeze(0)).squeeze(0).mean(dim=0), dim=-1)
    
    def eval_gold():
        with torch.no_grad():
            c = 0
            for i, ei in enumerate(ge_r):
                hw_vec = F.normalize(hw.forward(torch.tensor(ei, device=device).unsqueeze(0)).squeeze(0).mean(dim=0), dim=-1)
                if (hw_vec.unsqueeze(0) @ gw_r.T / tau).squeeze(0).argmax() == i: c += 1
            return 100 * c / len(ge_r)
    
    gold0 = eval_gold()
    cos_init = sum(attn_paths.pairwise_cos()) / max(len(attn_paths.pairwise_cos()), 1)
    print(f"  gold init: {gold0:.1f}%  cos init: {cos_init:.4f}")
    
    trainable = (list(tree.parameters()) + list(hw.merge.parameters()) + 
                 list(hw.embedding.parameters()) + list(attn_paths.parameters()))
    opt = torch.optim.Adam(trainable, lr=0.003)
    
    def infonce_loss(query, target_ai):
        logits = (query @ anchors.zh_world.T) / tau
        return F.cross_entropy(logits.unsqueeze(0) if query.dim()==1 else logits, 
                              torch.tensor([target_ai], device=device))
    
    EPOCHS = 500
    for ep in range(EPOCHS):
        random.shuffle(single_ai)
        pnames = attn_paths.names()
        random.shuffle(pnames)
        for bi in range(0, max(len(single_ai), 1), 16):
            opt.zero_grad()
            l = torch.tensor(0.0, device=device); n = 0
            batch_paths = []  # collect target paths for masking
            for ai in single_ai[bi:bi+16]:
                en_ids = torch.tensor(anchors[ai][2], device=device)
                hw_vec = F.normalize(hw.forward(en_ids.unsqueeze(0)).squeeze(0).mean(dim=0), dim=-1)
                l += infonce_loss(hw_vec, ai); n += 1
                if use_mask:
                    zh_id0 = anchors[ai][3][0]
                    batch_paths.append([tree.route(torch.tensor([zh_id0], device=device))[l].item() for l in range(td)])
            m_start = (bi * 2) % max(1, len(pnames)-2)
            for pname in pnames[m_start:m_start+2]:
                _, _, ai = attn_paths.meta[pname]
                en_id = anchors[ai][2][0]
                cw = attn_paths.world_pos(pname, hw.embedding, en_id)
                l += infonce_loss(F.normalize(cw, dim=-1), ai); n += 1
                if use_mask and ai in zh_paths:
                    batch_paths.append(zh_paths[ai])
            if n > 0: 
                (l/n).backward()
                torch.nn.utils.clip_grad_norm_(trainable, 1.0)
                if use_mask and not freeze_tree:
                    tree.apply_mask(batch_paths, mask_radius)
                opt.step()
        
        if ep % 50 == 0 or ep == EPOCHS - 1:
            with torch.no_grad():
                gold = eval_gold()
                correct = 0; total = 0; ranks = []
                for pname in attn_paths.names():
                    _, _, ai = attn_paths.meta[pname]
                    en_id = anchors[ai][2][0]
                    cw = F.normalize(attn_paths.world_pos(pname, hw.embedding, en_id), dim=-1)
                    logits = (cw.unsqueeze(0) @ anchors.zh_world.T / tau).squeeze(0)
                    rank = (logits > logits[ai]).sum().item() + 1
                    ranks.append(rank)
                    if rank <= 1: correct += 1
                    total += 1
                ms_acc = 100 * correct / max(total, 1)
                avg_rank = sum(ranks) / max(len(ranks), 1)
                cos_pairs = attn_paths.pairwise_cos()
                avg_cos = sum(cos_pairs) / max(len(cos_pairs), 1)
                elapsed = time.time() - t0
                # Node pairwise cos at L4 (deepest, should show most differentiation)
                if not freeze_tree:
                    l4_cos = F.cosine_similarity(
                        tree.embeddings[4].weight[:, None, :],
                        tree.embeddings[4].weight[None, :, :], dim=-1
                    )
                    mask_l4 = ~torch.eye(16, dtype=torch.bool, device=device)
                    avg_l4_cos = l4_cos[mask_l4].mean().item()
                else:
                    avg_l4_cos = -1
                print(f"  ep {ep:4d} gold={gold:.1f}% ms={ms_acc:.0f}% rank={avg_rank:.1f} cos={avg_cos:.4f} L4cos={avg_l4_cos:.4f} {elapsed:.0f}s")
    
    gold1 = eval_gold()
    cos_final = sum(attn_paths.pairwise_cos()) / max(len(attn_paths.pairwise_cos()), 1)
    return {'name': name, 'gold0': gold0, 'gold1': gold1, 'cos_init': cos_init, 'cos_final': cos_final,
            'ms_acc': ms_acc, 'avg_rank': avg_rank}

results = []

# Exp 1: 冻结 trained tree + 3191 独立 path (基线) — 用 Paths 而非 AttnPaths
from core import Paths
print("=" * 60)
print("EXP 1: freeze trained tree + 独立 128D paths (基线)")
print("=" * 60)

tree1 = TreeNodes(dim=d, depth=td, init='zero', combine='add', vocab=V).to(device)
ckpt = torch.load('/mnt/nas/datasets/wmt17/checkpoints/anchor_tree_nce.pt', map_location=device, weights_only=True)
for i, tn in enumerate(tree1.embeddings): tn.load_state_dict(ckpt['t_nodes'][i])
for p in tree1.parameters(): p.requires_grad = False

hw1 = HeapWorld(vocab=V, dim=d, tree=tree1).to(device)
hw1.embedding.load_state_dict(ckpt['L0']); hw1.merge.load_state_dict(ckpt['t_merge'])
for p in hw1.embedding.parameters(): p.requires_grad = False
for p in hw1.merge.parameters(): p.requires_grad = False

anchors1 = AnchorIndex()
multi_en1 = set(multis.keys())
for e, z in ALL:
    if e.lower() in multi_en1: continue
    ei = sp.encode_as_ids(e); zi = sp.encode_as_ids(z)
    if ok(ei) and ok(zi): anchors1.add(e, z, ei, zi)
paths1 = Paths(dim=d).to(device)
for en, zhs in multis.items():
    en_ids = sp.encode_as_ids(en)
    if not ok(en_ids): continue
    for pi, zh in enumerate(zhs[:3]):
        zh_ids = sp.encode_as_ids(zh)
        if ok(zh_ids):
            anchors1.add(en, zh, en_ids, zh_ids)
            paths1.add(en, zh, anchors1.find(en)[-1], pi)
def _hw1_fn(tok_ids):
    return hw1.forward(torch.tensor(tok_ids, device=device).unsqueeze(0)).squeeze(0)
anchors1.build_zh_world(_hw1_fn)
single1 = [ai for ai, (en, _, _, _) in enumerate(anchors1.entries) if en not in multi_en1]
print(f"  anchors={len(anchors1)} paths={paths1.num_paths}")

ge1 = [sp.encode_as_ids(e) for e,_ in GOLD if ok(sp.encode_as_ids(e))]
gz1 = [sp.encode_as_ids(z) for _,z in GOLD if ok(sp.encode_as_ids(z))]
gw1 = torch.zeros(len(gz1), d, device=device)
with torch.no_grad():
    for i, zi in enumerate(gz1):
        gw1[i] = F.normalize(hw1.forward(torch.tensor(zi, device=device).unsqueeze(0)).squeeze(0).mean(dim=0), dim=-1)
def eval_gold1():
    with torch.no_grad():
        c = 0
        for i, ei in enumerate(ge1):
            hw_vec = F.normalize(hw1.forward(torch.tensor(ei, device=device).unsqueeze(0)).squeeze(0).mean(dim=0), dim=-1)
            if (hw_vec.unsqueeze(0) @ gw1.T / tau).squeeze(0).argmax() == i: c += 1
        return 100 * c / len(ge1)

gold0_1 = eval_gold1()
cos0_1 = sum(paths1.pairwise_cos()) / max(len(paths1.pairwise_cos()), 1)
print(f"  gold init: {gold0_1:.1f}%  cos init: {cos0_1:.4f}")

opt1 = torch.optim.Adam(paths1.parameters(), lr=0.01)
for ep in range(400):
    random.shuffle(single1)
    for bi in range(0, max(len(single1), 1), 16):
        opt1.zero_grad()
        l = torch.tensor(0.0, device=device); n = 0
        for ai in single1[bi:bi+16]:
            en_ids = torch.tensor(anchors1[ai][2], device=device)
            hw_vec = F.normalize(hw1.forward(en_ids.unsqueeze(0)).squeeze(0).mean(dim=0), dim=-1)
            logits = (hw_vec.unsqueeze(0) @ anchors1.zh_world.T) / tau
            l += F.cross_entropy(logits, torch.tensor([ai], device=device)); n += 1
        pnames = paths1.names()
        m_start = (bi * 2) % max(1, len(pnames)-2)
        for pname in pnames[m_start:m_start+2]:
            _, _, ai = paths1.meta[pname]
            en_id = anchors1[ai][2][0]
            cw = paths1.world_pos(pname, hw1.embedding, en_id)
            logits = (F.normalize(cw, dim=-1).unsqueeze(0) @ anchors1.zh_world.T) / tau
            l += F.cross_entropy(logits, torch.tensor([ai], device=device)); n += 1
        if n > 0: (l/n).backward(); torch.nn.utils.clip_grad_norm_(list(paths1.parameters()), 1.0); opt1.step()
    if ep % 100 == 0 or ep == 399:
        with torch.no_grad():
            gold = eval_gold1()
            correct=0; total=0; ranks=[]
            for pname in paths1.names():
                _,_,ai=paths1.meta[pname]
                en_id=anchors1[ai][2][0]
                cw=F.normalize(paths1.world_pos(pname,hw1.embedding,en_id),dim=-1)
                logits=(cw.unsqueeze(0)@anchors1.zh_world.T/tau).squeeze(0)
                rank=(logits>logits[ai]).sum().item()+1
                ranks.append(rank)
                if rank<=1:correct+=1
                total+=1
            ms=100*correct/max(total,1)
            avg_rank=sum(ranks)/max(len(ranks),1)
            cos_p=paths1.pairwise_cos()
            avg_cos=sum(cos_p)/max(len(cos_p),1)
            print(f"  ep {ep:4d} gold={gold:.1f}% ms={ms:.0f}% rank={avg_rank:.1f} cos={avg_cos:.4f} {time.time()-t0:.0f}s")

results.append({'name':'independent_paths','gold':eval_gold1(),'ms':ms if locals().get('ms') else 0,'cos':avg_cos if locals().get('avg_cos') else 0})
print(f"  done")

# Exp 2: unit_root×mul tree + AttnPaths + 有序掩码
r2 = run_experiment("unit_root+AttnPaths+mask(radius=2)",
                     TreeNodes(dim=d, depth=td, init='unit', combine='mul', vocab=V).to(device),
                     freeze_tree=False, use_mask=True, mask_radius=2)
results.append(r2)

# Exp 3: unit_root×mul tree + AttnPaths + 有序掩码(radius=4) 
r3 = run_experiment("unit_root+AttnPaths+mask(radius=4)",
                     TreeNodes(dim=d, depth=td, init='unit', combine='mul', vocab=V).to(device),
                     freeze_tree=False, use_mask=True, mask_radius=4)
results.append(r3)

# Exp 4: unit_root×mul tree + AttnPaths + NO mask (对照)
r4 = run_experiment("unit_root+AttnPaths+NOmask",
                     TreeNodes(dim=d, depth=td, init='unit', combine='mul', vocab=V).to(device),
                     freeze_tree=False, use_mask=False, mask_radius=0)
results.append(r4)

print("\n" + "=" * 60)
print("夜间实验结果汇总")
print("=" * 60)
for r in results:
    rname = r.get('name', r.get('name','?'))
    if 'gold0' in r:
        print(f"  {rname:40s} gold: {r['gold0']:.1f}→{r['gold1']:.1f}  ms={r['ms_acc']:.0f}%  cos: {r['cos_init']:.4f}→{r['cos_final']:.4f}")
    else:
        print(f"  {rname:40s} gold={r.get('gold','?'):.1f}  ms={r.get('ms',0):.0f}%  cos={r.get('cos',0):.4f}")

print(f"\nTotal time: {time.time()-t0:.0f}s")

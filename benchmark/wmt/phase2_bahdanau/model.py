...
class AttnDecoder(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers=2, dropout=0.3, dual_head=False):
        super().__init__()
        ...
        if dual_head:
            self.out_sb = nn.Linear(hidden_size, vocab_size)  # SoftBLEU head

    def forward(self, tgt, encoder_states):
        enc_out, (hidden, cell) = encoder_states
        embedded = self.embed(tgt)
        src_mask = (enc_out.sum(-1) == 0)
        outputs = []
        for t in range(embedded.size(1)):
            dec_h = hidden[-1]
            context, _ = self.attention(dec_h, enc_out, src_mask)
            rnn_input = torch.cat([embedded[:, t, :], context], dim=-1).unsqueeze(1)
            output, (hidden, cell) = self.rnn(rnn_input, (hidden, cell))
            outputs.append(output)
        hidden_seq = torch.cat(outputs, dim=1)  # (B, T, H)
        logits = self.out(hidden_seq)
        if self.dual_head:
            self._sb_logits = self.out_sb(hidden_seq)
...
class AttnSeq2Seq(nn.Module):
    ...
    def forward(self, src, tgt, src_len):
        ...
        if self.decoder.dual_head:
            return logits, self.decoder._sb_logits
        return logits

...
| M0-ROT-C02 | Rotation should be a fixed-capacity subheap self-map `R: H_C -> H_C`; shared local kernels may use registered rotations to transfer across addresses/orientations and expand whole-tree receptive field through bounded iterations, without allocating nodes. | design; protocol-carrier component supported | `logic/fixed_capacity_subheap_rotation.md`; `evidence/fixed_capacity_rotation_protocol_probe/`: paired echo MSE `0`, cross MSE `2.012999`, capacity `127`; wrong-order gate mixed | Capacity or node count changes; inverse fails; orientation/address transfer fails; random permutations work equally well; or propagation requires dynamic allocation. |

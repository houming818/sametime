...
| M0-ROT-C01 | A bounded, order-preserving, invertible rotation plus recursive `CAT` can represent a doubled ordered TreeHeap orbit lazily, while one shared comparison kernel retrieves members in steps linear in rotation depth and logarithmic in logical candidate count. | design | Preregistered in `logic/bounded_rotation_search.md`; planned `evidence/bounded_rotation_search_probe/` | Exact retrieval fails within budget; query steps exceed the registered depth bound; lazy storage grows like explicit copies; or a non-order-preserving control works equally well with the same one-path kernel. |

...
| M0-ROT-C02 | Rotation should be a fixed-capacity subheap self-map `R: H_C -> H_C`; shared local kernels may use registered rotations to transfer across addresses/orientations and expand whole-tree receptive field through bounded iterations, without allocating nodes. | design | `logic/fixed_capacity_subheap_rotation.md` | Capacity or node count changes; inverse fails; orientation/address transfer fails; random permutations work equally well; or propagation requires dynamic allocation. |

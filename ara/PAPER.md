...
| C6-016 | SPR-043 | Real-sentence S1 echo should be evaluated with same-algebra TreeHeap perturbation/recovery and readable examples, not only toy token ids. | supported pilot | `ara/s1-echo/evidence/s1_sentence_flip_echo_probe/`; WMT17 English side, whitespace tokens, 20,000 samples, length 3-32, vocab 8192; perturbation is TreeHeap `Flip(root, full_depth)`. Hard closure exact `1.0000`; learned inverse OOD exact/token/edit `0.9645/0.9978/0.9979`; no-inverse OOD exact `0.0000`; examples included. | Hard closure fails, perturbation is external array reverse, examples are absent, no-inverse/matched baselines catch up, or local `Flip(node, depth)` tasks collapse. |
...
1. `Learned Echo Encoder/Decoder`
   ...
   - Next: replace root/full-depth flip with local `Flip(node, depth)` discovery and stronger matched baselines.

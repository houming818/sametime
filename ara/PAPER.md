...
| C7-012 | post SPR-075 | Exact conditional-count path NLL cannot learn TreeHeap placement, but finite-budget Hit@K can optimize fixed-capacity leaf arrangement when queries share candidate structure. | supported controlled index toy / natural-language claim open | `ara/s3-generation/evidence/s3_stone1_c15_budgeted_index/`; exact NLL layout range `6.66e-16`, flat/tree storage `192/372`, random/optimized held-out Hit@3 `0.6921/0.8125`, shuffle advantage drop `0.0363`; io task 62 | Retain topology-invariance theorem and finite-budget placement evidence. Require bounded node states and matched real-corpus flat-index comparison before connecting to generation. |

...
状态：初始 smoke 与 r1 均已完成。r1 修复递归因果位置后 P0/P1/P2/P5 通过，P3/P4 未支持；不进入正式长训。
...
taskd `390` 在 RTX 3090 上完成两臂各 30 outer steps，用时 175.2 秒。实验使用同一
106M checkpoint，冻结参数 `105,965,594`，新增退火参数 `30,890`。step 0 文本逐 token
一致、WMT valid NLL 差为 0；训练期间功率上限保持 `270 W`，未见 OOM、NaN/Inf 或 GPU
故障，保存后重载评价差为 0。因此 P0/P2 继续通过。

| 状态 | train positive | test positive | test loss | 滤镜遗憾值 |
|---|---:|---:|---:|---:|
| step 0 | 0.631262 | 0.271733 | 2.472587 | 0.274638 |
| direct theta | 0.629189 | 0.272722 | 2.418750 | 0.288187 |
| filter-guided theta | 0.629306 | 0.273629 | 2.415589 | 0.250932 |

同位置滤镜在 step 0 平均可降低行为损失 `0.274638`，P1 通过。guided 训练后遗憾值相对
step 0 降低 `0.023706`（约 `8.63%`），而 direct 反而增加 `0.013550`，所以 P5 通过：
直接拟合滤镜修正的辅助目标确实让当前径向 `theta` 吸收了一小部分局部方向。这也说明初始
smoke 的因果位置不匹配是真实实现问题，而不只是结果波动。

但能力门没有通过。guided 的 train positive 比起点低 `0.001956`，P3 失败；test positive
只比起点高 `0.001896`，且只比 direct 高 `0.000907`，远低于 P4 预注册的 `0.02` 增量。
三条 held-out 句在 direct 与 guided 两臂中的贪心生成逐字相同；“西西弗每天都推着石头上
山顶”仍未生成 `push`，而是继续生成 hill、mountain、stone 等邻近内容。guided 与 direct
的 WMT valid mean NLL 分别为 `4.214449` 和 `4.214461`，差 `0.000012`，在本 smoke 的
16 行评价规模下不作质量结论。

因此 r1 支持的严格结论是：**滤镜能够为可训练退火参数提供可吸收的局部校准信号，但当前
标量径向参数化和八条词汇守恒训练样本不足以把该信号转化为新的组合生成能力。** 它不支持
“已经找到理想参数 theta”，也不支持只增加训练步数就能恢复 `push`。

## 11. 下一步边界

本版本不启动 formal。直接延长相同目标会把 GPU 时间用于放大一个尚未越过 argmax 决策边界
的弱信号。下一阶梯应保持 checkpoint、样本与评价合同，单独增加 `theta` 的方向自由度：从
每个 parent 的标量径向 gain，升级为低秩残差向量或通道组 gain，使滤镜不只回答“放大或缩小
多少”，还能表达“隐态应向哪个词汇约束方向旋转”。先做同规模配对 smoke；只有 train
positive 守恒、held-out 生成发生可复现变化且 P4 通过，才进入正式训练。

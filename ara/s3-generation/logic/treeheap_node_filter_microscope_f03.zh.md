...
taskd `379--383` 反查 checkpoint 合同并流式扫描了它实际训练使用的完整语料：

```text
/home/nio/datasets/nio/releases/NioClean-ZHEN-S098-7M-v2/pairs.tsv
```

该文件有 7,304,358 对中英句子、1.54 GB，无坏行。`push*` 并不稀缺，但分布高度分散：
...
| `pushed` | 1,291 |
| `pushes` | 372 |
| 合计 | 8,022 次，分布于 7,704 行 |

在英文 `push*` 对齐句中，只有 4,618/7,704 的中文侧包含字面“推”，约 59.9%。反方向看，
中文侧含“推”的句子共有 80,523 行，只有 4,618 行对齐到 `push*`，约 5.7%。常见对应还包括
“推动、推进、推送、推广、按、压、迫使”等不同动作与抽象结构。

`push*` 后最常见的词包括 `the(1179)`、`forward(441)`、`pull(391)`、`for(368)`、
`to(367)`、`button(237)` 和 `up(171)`。这说明训练信号大量来自技术操作、软件推送、
`push forward`、`push for`、按钮和抽象推进，而不是人物推动具体物体。

与本标本直接相关的组合非常稀疏：

- 同行含 `push*` 和 `stone/rock/boulder` 的只有 36 行，其中不少只是两个词偶然出现在长句；
- `he/Sisyphus + push* + stone/rock/boulder` 的近邻模式只有 3 行；
- 自然中文 `推着...石头/石块/巨石/岩石` 的近邻模式为 0 行；
- Sisyphus 相关共 9 行，Sisyphus 与 `push` 同现只有 1 行；
- 唯一完整接近的英文是 `Sisyphus was punished ... to push the same rock up a hill`，但中文侧
  是 `Sisyphus西西弗斯 was punished处罚 ...` 这种英文逐词夹注，并非自然中译英样本；
- 全库 `pushes` 虽有 372 次，但检出的唯一 `pushes` 与 `stone/rock` 同句样本实际是
  `child pushes back or rocks`，不是第三人称“推石头”。

因此 F03 与重复探针需要加入一个重要混杂因素：模型已经从大量分散语境中学会孤立
`推 -> push/pushing`，所以重复“推”能够放大并显影；但语料几乎没有教过自然中文条件下的
“人物 + 第三人称推动 + 石头 + 上山”组合。当前完整句失败既可能来自压缩/READ 的组合干扰，
也可能来自训练分布缺口，不能只归因于 TreeHeap。

这也解释了为什么重复“推石头”时 `stone` 赢得输出而 `push` 消失：`stone` 的名词映射较
直接，而“推”对应的英文协议被技术、抽象、祈使、被动和进行时语料分散。下一步若要区分
架构缺陷与语料缺口，需要用少量、结构受控的自然平行句补齐这一组合，再用同一显微镜复测。

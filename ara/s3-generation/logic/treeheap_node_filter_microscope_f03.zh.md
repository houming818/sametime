...
为检查第 8 节是否只是 `push` 的个例，taskd `384` 流式扫描同一份 7.3M 平行语料，taskd
`385` 使用同一冻结 checkpoint、同一中译英 Decode 路径，在 depth `5/6/7` 比较裸词、
重复裸词、动宾组合和主谓宾组合。这里的 `like` 不是干净的及物动词标签：语料中的它还包括
“像、例如、看起来像”等用法；`want` 也主要进入 `want to + verb`，因此总频次不能直接当作
某个主谓宾结构的训练量。

### 10.1 语料分布

| 英文词族 | 命中行 | 词形次数 | 中文字面标记覆盖 | 最常见后接结构 |
|---|---:|---:|---:|---|
| `push*` | 7,704 | 8,022 | 60.5% 含“推/按压” | `the`, `forward`, `pull`, `for`, `to` |
| `like*` | 108,437 | 113,535 | 13.9% 含“喜欢/喜爱/爱好” | `to`, `a`, `the`, `this`, `that` |
| `want*` | 82,008 | 85,782 | 70.3% 含“想要/希望/想” | `to` 占 62,679 次，其后才是冠词和代词 |

`like*` 的 13.9% 低字面覆盖主要说明词义混杂，不表示其对齐质量只有 13.9%。`want*` 虽然
丰富，但约 73.1% 的出现直接后接 `to`；训练最强的协议更接近“某人想做某事”，不是
“某人想要某物”。`push*` 则同时分散在实体推动、技术操作、软件推送和抽象推进中。

### 10.2 冻结模型直接 Decode

下表只摘录能区分现象的输出；同一输入的三种 depth 均已执行：

| 输入类型 | `push` | `like` | `want` |
|---|---|---|---|
| 单个裸词 | `推。` 未出现 `push` | `喜欢。` 在 depth 5 得到 `I like.` | `想要。` 三个 depth 均失真，未出现 `want` |
| 裸词重复 8 次 | 显著出现并重复 `push/pushing` | 显著出现并重复 `like/love` | 显著出现并重复 `want to` |
| 动词+苹果重复 | `推石头` 保留 `stone`，丢失 `push` | 保留 `apple/like`，但生成 `Apple's like...` 等错误关系 | depth 6/7 出现 `apple wants...`，主客体角色倒置 |
| 小明+动词+苹果重复 | 保留若干实体词，丢失推动关系 | 生成 `The small apples like the apple.` 等错误绑定 | depth 7 生成 `small children want...`，但未稳定恢复“小明想要苹果” |

三个词族由此呈现两层规律：

1. **词汇通路存在。** 重复裸词都能把对应英文词形推过 Decode 阈值；`like` 甚至单次可读。
2. **组合通路不稳定。** 加入宾语或主语后，动作/态度词可能消失，也可能保留却绑定到错误
   角色；常见失败不是“一个词都没有”，而是 `apple wants`、`apples like` 这样的结构换位。

因此，`push` 的现象不是孤例，但三个词也并非完全同构。`push` 的主要困难同时包含训练组合
稀疏和词义分散；`like` 词汇读出最强，却受多义性与角色绑定影响；`want` 的语料量很大，
但强先验是 `want to`，它会在重复探针中压过直接宾语结构。当前证据更支持：**冻结模型已经
保存若干可放大的词汇方向，却没有稳定保存或读出“主语-谓语-宾语”的联合约束。**

这仍不能把故障唯一归因于 TreeHeap，因为训练语料的结构比例不同。下一次控制实验应选取
相同人物、相同对象、只替换动词的自然平行句，并分别统计目标结构的真实训练频次；在此之前，
不应根据单个词的总频次给架构定罪或宣告组合能力成立。

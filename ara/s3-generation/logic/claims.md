...
| S3-TREEHEAP-PYRAMID-C01 | A frozen TreeHeap root compressor can be extended into a multiresolution codec: root stores coarse task-relevant state and bounded addressed detail codes progressively restore local states, yielding a monotonic rate-distortion curve without changing root next-token NLL. | designed / experiment pending | Design: `multiresolution_treeheap_pyramid.md`, 2026-07-14. Starting checkpoint: the finite four-head no-residual model from `evidence/s3_residual_treeheap_forest/full/`. First gate uses 1M blocks, detail widths `0/8/16/32/64`, three seeds, fixed/flat/random-address controls, and a frozen root model. | Reject or narrow if reconstruction is not monotonic, k=64 cannot halve root-only MSE, random pairing matches trained addresses, equal-rate flat/mean controls dominate, or root NLL changes through a bypass. Do not claim entropy-coded compression, scale specialization, or TreeHeap advantage until their separate gates pass. |

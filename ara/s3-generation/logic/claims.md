...
| ID | Claim | Status | Evidence | Falsification |
|----|-------|--------|----------|---------------|
| S3-RESIDUAL-FOREST-C01 | A preallocated multi-head parameter TreeHeap forest can be trained on real text through a root-only next-token objective, and the native residual update `H_next = H plus DeltaH` should preserve useful state and gradients better than a same-parameter no-residual TreeHeap while measurably using left/right addresses and more than one kernel head. | running / smoke mixed | Design: `residual_treeheap_forest_pretrain.md`; script: `s3_residual_treeheap_forest_pretrain.py`; queue: `run_residual_treeheap_forest_queue.sh`; io systemd unit `ara-s3-residual-forest.service`, started 2026-07-13. Three 65K-block smoke seeds completed: matched residual NLL gain `+0.0338/-0.0127/+0.0891`; address-destruction NLL increase `0.0061/0.0082/0.0081`, below the `0.02` gate; all head-ablation and non-collapse gates passed. Full local-corpus blocks and training are in progress. Evidence target: `evidence/s3_residual_treeheap_forest/`; raw corpus and blocks stay on io's local `/home/nio/datasets`, with final evidence backed up to NAS. | Reject or redesign if matched no-residual NLL is not worse across seeds/full corpus, address destruction has no NLL cost, all heads are intervention-equivalent, root states collapse, a leaf bypass exists, or bounded and complete-corpus results disagree. Passing does not establish consciousness, world knowledge, WMT superiority, or automatic semantic categories. |

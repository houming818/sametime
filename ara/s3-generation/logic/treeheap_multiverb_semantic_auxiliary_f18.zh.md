...
状态：正式修订版已完成；辅助语义方向未达到预注册迁移门，保留为开放/否定性结果。
...
修订版使用 21 个满足 token 化容量合同的表达族，训练/评估实例为 `1344/336`，完成 head warmup 50
steps 与 joint 120 steps。所有运行合同通过：数据合同、有限 loss/梯度、source 冻结、模型确实更新、
checkpoint reload 和 WMT 保持门均通过；正式 checkpoint 未拉回 Git，仅保留远端路径指针。

- 辅助验证：root 宏平均 AUROC `0.5936 -> 0.6197`，leaf `0.5929 -> 0.6192`；两层差值约 `0.0004`，
  说明在这次设置下没有观察到 root 相对于 leaf 的明显额外退化；
- P1 未通过：root `0.6197 < 0.65`；P2 通过；
- F16 完全排除的 `push*` 留出集：FOLD root `+0.0012`，READ-facing root `+0.0061`，均低于 `+0.03`
  迁移门，因此 P3/P4 未通过；
- WMT 对照 mean NLL `4.1537 -> 4.1044`，变化 `-0.0493`，S0 通过，没有观察到翻译损伤；
- 按预注册诊断，结果为“辅助压力尚未形成足够强的语义方向”，不是“FOLD 已被证伪”，也不是“语义无法形成”。

本结果支持一个较窄的判断：在当前 21 族、自动 target-presence 标签、线性共享 head、`0.20` 辅助权重和
170 steps 的组合下，模型能在训练族上获得有限的方向改善，但没有形成可迁移到排除族的统一方向。由于
WMT NLL 同时改善，不能把失败归因于训练破坏了主翻译目标。下一步若继续，应优先增加语义监督的质量/覆盖
或改变读出模型，并保留 F16 留出作为迁移检验；不应仅凭本次结果替换 FOLD 退火算子。

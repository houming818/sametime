...
状态：smoke 已完成；工程合同与 P1--P3 均通过，支持局部多方向场，尚未形成可推理路由。
...
io taskd 404 在冻结的 TreeHeap-106M checkpoint 上完成 32 条双向 WMT held-out 样本、三个
protocol depth 的审计，用时 21.76 秒。零坐标 logits 与逐样本 NLL 都和 native 精确相同；
最大成对能量相对误差为 `2.35e-7`，梯度有限且非零，checkpoint 状态未改变。O0--O2 全部
通过。GPU 保持 270 W 上限，观测峰值功率 165.86 W、峰值温度 57 C、峰值显存 1242 MiB。

整体 80 维局部方向矩阵得到：

| protocol depth | 第一方向解释率 | effective rank | 负余弦比例 | scalar 能量比例 |
|---:|---:|---:|---:|---:|
| 5 | 0.4471 | 3.8988 | 0.4879 | 0.2809 |
| 6 | 0.3563 | 5.1199 | 0.5000 | 0.3058 |
| 7 | 0.3744 | 5.2412 | 0.4556 | 0.3013 |

三个 depth 的样本梯度都没有集中在一根方向轴上。全体样本加权后，同步 scalar 子空间只保留
`0.3003` 的梯度能量；约七成局部方向信息位于组间差异中。逐 merge depth 也复现了这个现象：
所有有效单元第一方向解释率为 0.32--0.76，effective rank 为 1.70--4.71，负余弦比例为
0.42--0.55。因此 P1/P3 通过，不能把结果只解释成不同 protocol depth 之间互相抵消。

有效 merge 范围随协议分辨率扩大：depth 5 只在 merge 0--2 有非零梯度，depth 6 覆盖
merge 0--3，depth 7 覆盖 merge 0--4。这来自可用 slot/budget 对双子节点的支持范围，而不是
高层梯度数值消失。它支持“递归深度首先是一个位置与有效支撑域”的解释。

同 raw-coordinate L2 位移预算的有限前向复核如下：

| depth | 步长 | grouped - native NLL | scalar - native NLL | grouped - scalar | grouped 胜率 |
|---:|---:|---:|---:|---:|---:|
| 5 | 0.5 | -0.04437 | -0.02098 | -0.02339 | 1.00 |
| 5 | 1.0 | -0.07968 | -0.03701 | -0.04267 | 1.00 |
| 6 | 0.5 | -0.06749 | -0.03132 | -0.03616 | 1.00 |
| 6 | 1.0 | -0.12344 | -0.05823 | -0.06522 | 1.00 |
| 7 | 0.5 | -0.11310 | -0.05451 | -0.05859 | 1.00 |
| 7 | 1.0 | -0.20442 | -0.10370 | -0.10073 | 1.00 |

两档步长、三个 depth 上，grouped 都在 32/32 样本中优于 scalar，P2 通过。这说明 SVD 所见
的额外方向在真实非线性前向中可被利用，不只是数值分解产生的图形。

## 7. 当前结论

F12 支持一个比“概率场是一个多边形”更准确的说法：**在当前 checkpoint、当前 8 组坐标尺度
和目标函数下，同一 TreeHeap 递归位置的局部可改进方向是多轴、带冲突的，呈多面体式局部
几何；单个同步旋转角只能覆盖约三成梯度能量。**

这为 F11 的失效提供了结构解释，但还不是解决方案。grouped 方向由每个样本的正确 target
反传得到，属于 oracle 显微镜结果。下一步需要测试一个能够只根据 `(left, right, depth)` 预测
分组方向的低秩路由头，并与同参数量 scalar 头配对；不能把逐样本 oracle 坐标直接写进模型，
也不能据此宣称 literal polygon、翻译提升或默认 FOLD 已可替换。

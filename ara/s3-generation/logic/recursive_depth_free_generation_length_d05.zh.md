...
状态：`completed / natural shorter-at-shallow-depth not supported`
...
taskd `322` 在 `151.8s` 内完成三 seed 正式实验。三个 checkpoint 使用同一批 128 条
test rows，每条输入在每个深度执行 1 次 greedy 与 8 次 temperature `0.8`、top-p
`0.9` 采样。共同生成上限为 256，生成过程没有读取 target。

三 seed 的随机采样结果：

| seed | depth 5 中位长度 / 触顶率 | depth 6 中位长度 / 触顶率 | depth 7 中位长度 / 触顶率 |
|---:|---:|---:|---:|
| 10101 | 256 / 80.47% | 13 / 6.15% | 12 / 0.20% |
| 10102 | 256 / 87.50% | 14 / 6.93% | 13 / 0.68% |
| 10103 | 256 / 87.50% | 11 / 6.25% | 11 / 0.00% |

三 seed 平均诊断：

| READ 深度 | 长度均值 | 中位长度 | 触顶率 | 相邻重复率 | BLEU4（辅助） |
|---:|---:|---:|---:|---:|---:|
| 5 | 223.58 | 256.00 | 85.16% | 49.7% | 0.21 |
| 6 | 32.66 | 12.67 | 6.45% | 22.5% | 3.25 |
| 7 | 18.66 | 12.00 | 0.29% | 3.7% | 5.82 |

P0 通过；P1、P2、P3、P4 均未通过。三个 seed 中，配对样本满足
`L5 <= L6 <= L7` 的比例只有 `12.50%` 至 `13.18%`，严格单调比例全部为 `0`。
greedy 也给出相同方向，因此结果不是随机采样偶然。

## 9. 对删失数据的严格解释

程序按预注册的保守规则输出 `inconclusive_length_censored`，因为 depth 5 的真实停止
长度大量超过 256，无法计算精确的 `L5/L7`。但这不意味着 P1 的方向仍然未知：

- 超过一半的 depth-5 样本长度已经确认至少为 256，因此其真实中位数不小于 256；
- depth-7 的中位数仅为 11 至 13，且触顶率不超过 0.68%；
- 所以 `median(L5) < median(L7)` 不可能成立。

因此，**精确长度比例因右删失而不可估计，但“较浅深度自然生成较短文本”的假设已被
否定。** 当前 checkpoint 呈现相反规律：浅层 READ 不能稳定形成 EOS，生成会进入
高频 token 循环；随着递归深度开放，EOS、低重复率和可读内容才逐步恢复。

## 10. 当前含义

D05 没有证明 TreeHeap 的 coarse state 对应短句。它证明了另一个更基础的事实：当前
私有协议把完整递归 READ 当作维持生成轨迹稳定性的条件。depth 6 与 depth 7 的中位
长度相近，但 depth 6 具有更长的尾部、更高重复率和更低 BLEU；depth 5 则大面积失去
停止能力。

所以，现阶段不能把“语言分辨率”直接定义为“生成 token 数”。若继续探索缩句游戏，
需要显式训练目标侧的递归 coarse state，再独立评价语义保留；不能仅截断 READ 深度并
期待 Decoder 自动缩短句子。

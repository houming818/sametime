...
RUNNER="$ROOT/ara/s3-generation/scripts/run_stone1_c02_formal.sh"
EVIDENCE="$ROOT/ara/s3-generation/evidence/s3_stone1_canonical_codec"
LAUNCH_LOG=/tmp/s3_stone1_c02_formal_launcher.log
...
if pgrep -af "[r]un_stone1_c02_formal.sh" >/dev/null; then
  echo "formal runner is already active"
  pgrep -af "[r]un_stone1_c02_formal.sh"
  exit 0
fi

...
runner="ara/s3-generation/src/s3_recursive_depth_free_generation_length.py"
root="ara/s3-generation/evidence/s3_recursive_depth_free_generation_length_d05"
...
if not row["gates"]["P0"]:
    raise SystemExit("D05 smoke P0 failed")
if row["seeds"] != [10101]:
    raise SystemExit("D05 smoke seed mismatch")
PY
...
row = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
if not row["gates"]["P0"]:
    raise SystemExit("D05 formal P0 failed")
print(json.dumps({
    "event": "D05_complete",
    "gates": row["gates"],

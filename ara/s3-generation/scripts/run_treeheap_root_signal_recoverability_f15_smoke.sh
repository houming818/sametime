...
evidence="ara/s3-generation/evidence/s3_treeheap_root_signal_recoverability_f15/smoke_seed12601"
mkdir -p "$evidence"

command=(
  python3 ara/s3-generation/src/s3_treeheap_root_signal_recoverability_f15.py
  ...
  --device cuda
  --ridge 0.001
  --permutation-seed 12601
)
printf '%q ' "${command[@]}" >"$evidence/command.txt"
printf '\n' >>"$evidence/command.txt"
sha256sum \
  ara/s3-generation/src/s3_treeheap_root_signal_recoverability_f15.py \
  ara/s3-generation/logic/treeheap_root_signal_recoverability_f15.zh.md \

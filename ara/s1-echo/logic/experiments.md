...
## P-S1-ANNEAL-SPACE01: Annealed FOLD/UNFOLD Token Space

Status: executed / supported controlled pilot
Claim: S1-ANNEAL-SPACE-C01
Design: `logic/annealed_fold_unfold_token_space.md`
Script: `src/s1_annealed_fold_unfold_token_space.py`
Evidence: `evidence/s1_annealed_fold_unfold_token_space/`

### Question

Can the token coordinate system be generated by TreeHeap itself, rather than
starting from a randomly placed trainable embedding table?

### Intervention

The observer converts token-context counts into probability distributions.
There is no learned token vector.  At each internal node, UNFOLD computes a
soft left/right assignment from Hellinger distance and temperature; FOLD
recomputes child prototypes from those responsibilities.  Cooling recursively
creates a route-probability coordinate.

### Controls

```text
structured observations
context-shuffled observations with equal per-token count
random routes over the structured observations
```

Labels are hidden from FOLD/UNFOLD and used only to audit whether routes recover
the controlled latent law.  Independent held-out observations test whether
leaf prototypes predict context rather than only partition train counts.

### Registered Gates

```text
purity gap vs shuffled >= 0.20
purity gap vs random route >= 0.20
pairwise-F1 gap vs shuffled >= 0.20
held-out NLL better than both controls
route stability better than shuffled
leaf utilization >= 0.75
normalized occupancy entropy >= 0.80
FOLD parent/children conservation max error <= 1e-6
```

Passing conservation without passing semantic/control gates is only numerical
closure.  It must not be reported as semantic token geometry.

### Result

Formal io task `446` ran 24 seeds (`19001..19024`) and produced 72 complete
arm records.  All values were finite and all nine preregistered gates passed.

```text
                         structured   shuffled   random_route
heldout context NLL          2.7745     8.6505         3.6886
cluster purity               1.0000     0.3991         0.3906
pairwise F1                  0.7438     0.0740         0.0850
route kNN top3               0.8902     0.1148         0.1105
leaf utilization             0.9948     0.9974         0.9714
occupancy entropy            0.9441     0.9760         0.9471
route stability              0.7094     0.2296         0.2368
```

Structured FOLD conservation max-abs mean was `1.02e-16`.  The important
control is that all arms occupied almost all leaves, while only the structured
arm recovered the latent law.  Purity `1.0` with pairwise-F1 `0.7438` means
every leaf was category-pure, but some categories were split across multiple
leaves; the pilot discovered separation but not a one-category/one-leaf code.

Decision:

```text
S1-ANNEAL-SPACE-C01 -> supported controlled pilot
```

The next gate is a real-corpus frequency-matched experiment with flat
clustering and SGNS/NCE baselines.  No WMT or generation claim follows from
this controlled result.

## P-S1-ANNEAL-SPACE-REAL01: Real-Corpus Annealed Token Space

Status: executed / supported real-corpus pilot with topology warning
Claim: S1-ANNEAL-SPACE-REAL-C01
Design: `logic/real_corpus_annealed_token_space.md`
Script: `src/s1_real_corpus_annealed_token_space.py`
Evidence: `evidence/s1_real_corpus_annealed_token_space/`

This is the real-data bridge for `S1-ANNEAL-SPACE-C01`.  It scans the English
side of WMT massive, forms token-context count matrices with the existing 32K
SentencePiece model, and reserves deterministic lines for held-out context
prediction.  No category label is available.

Matched controls are frequency-bin row permutation, random routes, flat
K-means with the same number of leaves, and an expected SGNS/NCE baseline.
The registered decision uses held-out NLL, bootstrap co-cluster stability,
SGNS neighbor overlap, leaf occupancy, and FOLD conservation.  Readable nearest
neighbors are evidence for inspection only and are not a pass condition.

### Result

io task `449` scanned 200,000 WMT massive English-side lines and produced
4,283,059 SentencePiece tokens, 6,599,588 train context pairs, and 735,369
deterministic held-out pairs.  Eight bootstrap seeds completed with finite
metrics.  All aggregate preregistered gates passed.

```text
held-out context NLL
  TreeHeap                 5.35895
  frequency shuffle       5.82061
  random route            5.46101
  flat K-means            5.36257
  expected SGNS           5.20325
  token empirical ref     5.20890
  global unigram ref      5.56978

co-cluster stability
  TreeHeap                 0.35096
  frequency shuffle       0.08583
  random route            0.03122
  flat K-means            0.59186

top-10 neighbor overlap with SGNS
  TreeHeap                 0.17148
  random route            0.02014
  flat K-means            0.12593
```

TreeHeap captures about `58.4%` of the held-out NLL improvement available
between the global reference and token-specific empirical reference, using a
five-component route coordinate.  Its NLL is statistically tied with flat
K-means under this pilot, while SGNS remains materially better.  TreeHeap is
less stable than flat K-means and seed `19105` partially collapses to
utilization `0.71875` and occupancy entropy `0.5093`.

Decision:

```text
S1-ANNEAL-SPACE-REAL-C01 -> supported real-corpus pilot
TreeHeap topology superiority             -> not supported
split/occupancy robustness                -> open
```

The next intervention should target the unstable bifurcation/occupancy rule,
not simply add more corpus or declare the current tree optimal.
...
## P-S1-TH-BACKGROUND01: TreeHeap Background Landmarks

**Status:** completed / BG2 point estimate supported / annealing branch collapse open
**Claim:** `S1-TH-BACKGROUND-C01`
**Design:** `treeheap_background_landmarks.md`

### Question

Can a fixed TreeHeap landing map, built only from long-run corpus statistics and
without current-sentence context, preserve measurable lexical background
association?

### Construction

- Corpus: first 200,000 valid English-side lines from the existing WMT massive
  training file.
- Tokenizer: the existing 32K SentencePiece model.
- Targets/contexts: top 8,192 / 4,096 corpus pieces.
- Observation: symmetric context window 4; every tenth source line is held out.
- Tree: depth 10, deterministic power-iteration split direction, soft annealed
  FOLD/UNFOLD responsibilities, three Poisson bootstrap builds.
- Matched baseline: 64D SGNS trained from the same count matrix.
- External ruler: SimLex-999; its labels are never used to construct the tree.

### Smoke

Use 20,000 lines, 1,024 targets, 512 contexts, depth 6, two bootstrap builds,
four EM steps, three power steps, and 100 SGNS steps.  Smoke checks execution,
finite values, evidence writing, conservation, and route variation only; its
reduced SimLex coverage cannot decide the claim.

### Formal Gates

- SimLex covered pairs >= 300.
- TreeHeap SimLex Spearman >= 0.12.
- TreeHeap rho exceeds random route by >= 0.05 and frequency-only by >= 0.03.
- TreeHeap retains >= 35% of positive matched-SGNS rho.
- TreeHeap held-out NLL beats global unigram and random routes.
- TreeHeap bootstrap stability beats random-route stability.
- Mean leaf utilization and normalized occupancy entropy are each >= 0.75.
- FOLD conservation max absolute error <= 1e-5.

### Interpretation Boundary

`BG1` means corpus association only. `BG2` additionally means agreement with
human lexical-similarity ordering beyond controls. `BG3` additionally requires
useful SGNS retention and stable, non-collapsed landmarks. This experiment does
not test facts, QA, sentence generation, context-conditioned senses, or world
knowledge in the strong sense.

### Result

Task 452 is retained as failed implementation evidence because its forced hard
splits and published soft routes disagreed. Corrected task 456 reached BG2:
Tree/random/global held-out NLL `6.4483/6.6039/6.7758`; SimLex-999 rho over 345
pairs Tree/random/frequency/Hellinger `0.1797/0.0210/0.1236/0.0483`. A 5,000
sample post-hoc bootstrap kept Tree-minus-random positive at the 95% lower
bound (`0.0064`) but Tree-minus-frequency crossed zero (`-0.0979..0.2071`).
Roughly nine tenths of internal nodes required deterministic fallback splits,
so fixed background landmarks are supported more strongly than the soft
annealing branch mechanism. Evidence:
`evidence/s1_treeheap_background_landmarks/formal_r1_200k_seed19201/`.
## P-S1-TH-EMBED-FP01: Single-TH Embedding Fixed Point

**Status:** completed / mechanism supported / quality advantage not supported
**Claim:** `S1-TH-EMBED-FP-C01`
**Design:** `th_embedding_fixed_point.md`

Train one token embedding table and one shared TreeHeap from fixed real-corpus
positive/negative pairs. The same TreeHeap transforms each token repeatedly;
one contrastive loss updates both parameter bodies. Smoke must pass the
gradient, finite-loss, held-out ordering, path-prefix, utilization, and entropy
gates before the bounded `1/2/4/8` repeat ladder is submitted.

### Result and follow-up

Smoke task 458 passed. Formal tasks 460--463 completed without runtime faults.
Repeated falls increased route utilization and entropy but did not beat the
matched embedding-only held-out accuracy. A bounded follow-up is registered:
r8 seeds `19302/19303` for 1,000 steps, plus r2/r8 seed `19301` for 5,000
steps, all from scratch on the same immutable cache. These runs distinguish
